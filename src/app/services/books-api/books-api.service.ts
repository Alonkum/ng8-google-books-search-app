...
export class BooksApiService {
  ...
  */
  ...
  list(startPosition: number = 0, title: string) : Observable<{items: Book[], totalItems: number}> {
  ...
  */
  search(query: string, startPosition: number): Observable<any> {
    ...
    return this.http.get<Book[]>(this.config.apiUrl, {params}).pipe(
      map((data: any) => {
        let res = {};
        res['items'] = data.items ?  this.convertCollection(data.items) : [];
        res['totalItems'] = data.totalItems;
        return res;
      })
    );
  }
